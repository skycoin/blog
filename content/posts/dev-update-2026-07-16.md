...
### Skycoin: A Minimum-Size TinyGo Binary

Two follow-ons to yesterday's TinyGo build. The release job now builds the TinyGo binary for minimum size with `-opt=z --no-debug`: keeping debug info and default optimization had left it at 47.6M — larger than the standard-go binary, defeating the point of shipping a TinyGo build — and the size flags drop it to ~32M, 31% smaller and below the standard-go build. And a golangci-lint sweep after the upstream merge greens the checks with `//nolint:errcheck` on the `webCtx` write helpers and `//nolint:gosec` on the node-proxy request paths (G704: they proxy to operator-configured node URLs, not user input) and the daemon pprof import (G108: profiling is only reachable when `--pprofmode=http` is set).
